...
class CallBackVideo implements ShouldQueue
{
    ...
    {
        LogHelper::add('data/callBackVideo', '当前数据:' . json_encode($this->data));
        ...
        // 获取sync_user updateby:video表mcn_id填充 2023-7-19
        ...
        try {
            $callBackService->haddleCallBackVideo($user, $sync, $this->data);
        } catch (OutPlatformException $e) {
            LogHelper::add('videoJobParamError/', '当前数据:' . json_encode($this->data));
            ...
            wxNotice('数据库查询异常', getErrorTemplateMessage($e) . '当前数据:' . json_encode($this->data));
        } catch (\PDOException $e) {
            wxNotice('视频解析log表主键冲突', getErrorTemplateMessage($e) . '当前数据:' . json_encode($this->data));
        } catch (\Throwable $e) {
            LogHelper::add('videoJobHaddleError/', getErrorTemplateMessage($e) . '当前数据:' . json_encode($this->data));
            wxNotice('视频解析异常', getErrorTemplateMessage($e) . '当前数据:' . json_encode($this->data));
        }
    }
}
