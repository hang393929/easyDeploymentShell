...
then
    cp ./php/config.json ~/.composer/config.json
fi
#导入配置
...
#docker-compose.yml不存在时创建软连接,这里不用线上的，直接就一个docker-compose.yml ：ln -s ./docker/docker-compose-prod.yml  ./docker-compose.yml
...
EOF
    ...
    echo "GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO '${DB_USERNAME}'@'%';" > ./mysql/init.sql
    echo $(echo $sql | sed -e 's/\r//g') >> ./mysql/init.sql
